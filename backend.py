...
#def create_table2():
 ...
 #   conn=psycopg2.connect("dbname='library_data' user='postgres' password='postgres1984' host='localhost' port='5432'")
  #  cur=conn.cursor()
   # cur.execute("select e1.firstname,e1.lastname,e1.employeeid,e1.entrytime,e2.exittime from entry1 e1 INNER JOIN entry2 e2 ON e1.firstname=e2.firstname AND e1.lastname=e2.lastname AND e1.employeeid=e2.employeeid")
...
def search(firstname="",lastname="",employeeid=""):
    ...
    #cur.execute("SELECT e1.firstname,e1.lastname,e1.employeeid,e1.entrytime,e2.exittime FROM entry1 e1 INNER JOIN entry2 e2 ON e1.firstname=e2.firstname AND e1.lastname=e2.lastname AND e1.employeeid=e2.employeeid WHERE firstname =e2.firstname OR e1.lastname =e2.lastname OR e1.employeeid=e2.employeeid")
    rows=cur.fetchall()
    conn.close()
    return rows
